 ...
 * （このクラスの場合{@link BuriStandardEngineImpl}）に登録されている必要があります。
 * このフローは、{@code /buri/dicon/buri-user.dicon}の
 * {@code "BuriEngineConfig"}に対してのコンポーネント定義で以下のように
 * 記述する事で登録ができます。
 * （以下の例では、権限主体に{@link ExcelBaseParticipantProvider}を指定しています。）
 * <pre>{@code
 ...
 * {@code /buri/dicon/buriStandard.dicon}です。
...
public interface StandardBuriProcessor {
     ...
     * {@code path}にそのデータが現在存在している状態のパスを指定、
     * さらに{@code userData}でこのデータを処理するユーザの情報を指定することで、
     * ユーザの権限も踏まえた状態遷移を実行することができます。
     * </p>
     * <p>
     * ユーザ情報を表すDtoを指定することで、フローを指定する際に一緒に指定した
     * {@link ParticipantProvider}がぶりで権限管理を行えるよう、
     * 内部で適切に処理をする事が可能となります。
     * </p>
     * 
    ...
    /**
     * 任意の{@link S2Container}を指定して、指定した状態に所属するデータの
     * IDのリストを返します。
     * 
     * @param path パッケージ名.プロセス名[.アクティビティ名]
     * @param userData 権限を伴うフローに対応したユーザ情報を表わすDto
