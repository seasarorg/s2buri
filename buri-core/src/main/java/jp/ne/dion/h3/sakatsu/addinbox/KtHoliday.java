...
public class KtHoliday {
	...
	public static String getHolidayName(Calendar prmDate) {
		Long dateID = Long.valueOf(prmDate.getTimeInMillis());
		...
		if (HolidayName.equals("")) {
			if (MyDate.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY) {
				...
				// 5/6(月)はここで判定する
				if (MyDate.after(cstImplementHoliday)
				    || MyDate.equals(cstImplementHoliday)) {
	...
	private static int prvDayOfSpringEquinox(int MyYear) {
		...
		if (MyYear <= 1947) {
			...
			if (MyYear <= 1979) {
				SpringEquinox_ret =
				    (int) (20.8357 + (0.242194 * (MyYear - 1980)) - ((MyYear - 1983) / 4));
			} else {
				if (MyYear <= 2099) {
					SpringEquinox_ret =
					    (int) (20.8431 + (0.242194 * (MyYear - 1980)) - ((MyYear - 1980) / 4));
				} else {
					if (MyYear <= 2150) {
						SpringEquinox_ret =
						    (int) (21.851 + (0.242194 * (MyYear - 1980)) - ((MyYear - 1980) / 4));
					} else {
	...
	private static int prvDayOfAutumnEquinox(int MyYear) {
		...
		if (MyYear <= 1947) {
			...
			if (MyYear <= 1979) {
				AutumnEquinox_ret =
				    (int) (23.2588 + (0.242194 * (MyYear - 1980)) - ((MyYear - 1983) / 4));
			} else {
				if (MyYear <= 2099) {
					AutumnEquinox_ret =
					    (int) (23.2488 + (0.242194 * (MyYear - 1980)) - ((MyYear - 1980) / 4));
				} else {
					if (MyYear <= 2150) {
						AutumnEquinox_ret =
						    (int) (24.2488 + (0.242194 * (MyYear - 1980)) - ((MyYear - 1980) / 4));
					} else {
