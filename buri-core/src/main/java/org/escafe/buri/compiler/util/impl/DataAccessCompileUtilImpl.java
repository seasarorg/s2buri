...
public class DataAccessCompileUtilImpl implements DataAccessCompileUtil {
	...
	public void setupDataAccessUtil(BuriDataAccessFactory factory,
			String className, BuriDataFieldType fieldType,
			BuriPackageType buriPackage, BuriWorkflowProcessType process,
			ClassLoader classLoader) {
		String processID = "";
		if (process != null) {
			processID = process.getId();
		}
		setupDataAccessUtil(factory, className, fieldType, buriPackage.getId(),
				processID, classLoader);
	}

	public void setupDataAccessUtil(BuriDataAccessFactory factory,
			String className, BuriDataFieldType fieldType, String packageId,
			String processId, ClassLoader classLoader) {
		Class clazz = ClassUtil.forName(className);
		DataAccessUtil accessUtil = compileDataAccess(fieldType, processId,
				packageId, classLoader);
	...
	protected void scriptSetup(BuriExePackages exePackage,
	...
	public DataAccessUtil compileDataAccess(BuriDataFieldType fieldType,
			BuriPackageType buriPackage, BuriWorkflowProcessType process,
			ClassLoader classLoader) {
		return compileDataAccess(fieldType, process.getId(), buriPackage
				.getId(), classLoader);
	}

	public DataAccessUtil compileDataAccess(BuriDataFieldType fieldType,
			String processId, String packageId, ClassLoader classLoader) {
		String createClassName = getDataAccessClassName(fieldType, processId,
				packageId);
		eventCaller.callStartCompile(this,
				BuriCompileEvent.CompileTargetType.DataField, createClassName);
		...
		eventCaller.callEndCompile(this,
				BuriCompileEvent.CompileTargetType.DataField, createClassName,
				util);
		return util;
	}

	private String getDataAccessClassName(BuriDataFieldType fieldType,
			String processId, String packageId) {
		String lastClassName = fieldType.getId().replaceAll("\\.", "_")
				+ "_DataAccessUtil";
		if (processId != null) {
	...
	private DataAccessUtilLongKey comileDataAccessUtilLongKey(
			BuriDataFieldType fieldType, String createClassName,
			ClassLoader classLoader) {
		...
		container.injectDependency(dataAccessUtil,
				AbstDataAccessUtilLongKey.class);
		return dataAccessUtil;
	}

	private DataAccessUtilManyKey comileDataAccessUtilManyKey(
			BuriDataFieldType fieldType, String createClassName,
			ClassLoader classLoader) {
		BasicCompileInfo info = new BasicCompileInfo();
		info.setOutputClassName(createClassName);
		...
		container.injectDependency(dataAccessUtil,
				AbstDataAccessUtilManyKey.class);
		return dataAccessUtil;
	}
	...
	private boolean isUseLongKeyType(BuriDataFieldType fieldType) {
		if (fieldType.getKeys().size() == 1) {
			...
			if (pkeyPropertyDesc.getPropertyType().isAssignableFrom(Long.class)) {
				return true;
			}
		}
		return false;
