...
public class DataAccessCompileUtilImpl implements DataAccessCompileUtil {
	...
	public void setupDataAccessUtil(BuriDataAccessFactory factory,
		...
		DataAccessUtil accessUtil =
		    compileDataAccess(fieldType, processId, packageId, classLoader);
		...
		if (accessUtil instanceof DataAccessUtilLongKey) {
			factory
			    .setDataAccessUtil(clazz, (DataAccessUtilLongKey) accessUtil);
		} else {
			factory
			    .setDataAccessUtil(clazz, (DataAccessUtilManyKey) accessUtil);
		}
	}
	...
	public DataAccessUtil compileDataAccess(BuriDataFieldType fieldType,
		...
		String createClassName =
		    getDataAccessClassName(fieldType, processId, packageId);
		eventCaller.callStartCompile(
		    this,
		    BuriCompileEvent.CompileTargetType.DataField,
		    createClassName);
		DataAccessUtil util = null;
		if (isUseLongKeyType(fieldType)) {
			util =
			    comileDataAccessUtilLongKey(
			        fieldType,
			        createClassName,
			        classLoader);
		} else {
			util =
			    comileDataAccessUtilManyKey(
			        fieldType,
			        createClassName,
			        classLoader);
		}
		eventCaller.callEndCompile(
		    this,
		    BuriCompileEvent.CompileTargetType.DataField,
		    createClassName,
		    util);
	...
	private String getDataAccessClassName(BuriDataFieldType fieldType,
		...
		String lastClassName =
		    fieldType.getId().replaceAll("\\.", "_") + "_DataAccessUtil";
		if (processId != null) {
