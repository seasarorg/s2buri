...
public class AbstBuriExeProcessDataAccess extends AbstBuriExecProcess implements
        BuriDataAccessFactory {
	...
	public void setup(BuriWorkflowProcessType process) {
		super.setup(process);
		DataAccessFactory rootFactory =
		    (DataAccessFactory) container.getComponent("rootDataAccessFactory");
	...
	protected BranchWalker setupStatus(String actId,
		...
		BranchWalker startWalker =
		    walker.moveNext(getActivityNameById(actId), actId);
		long statusID =
		    getStateUtil().loadStatus(this, sysContext, startWalker);
	...
	protected BranchWalker getSplitAndWalker(BuriSystemContext sysContext,
		...
		BranchWalker child =
		    getStateUtil().branchChild(walker, this, sysContext);
		return child;
	}
	...
	protected boolean canJoinAndFlow(BuriSystemContext sysContext,
	        BranchWalker walker, String nextName, String nextId) {
		long count =
		    getStateUtil().countNoProcessedSiblingStatus(
		        this,
		        sysContext,
		        walker);
		if (count == 0) {
