 ...
 * APIセットが提供されていますが、このクラスはその両方のプロセッサをサポートする為のAPIを提供しています。 このクラスより高いレイヤのAPIは
 * {@code Bao}と派生した拡張APIとして {@link BuriAutoSelectAutoSignalProcessorImpl}があります。
 * </p>
 * <p>
 * 具体的には、このクラス（の実装クラスである{@link BuriAutoSelectProcessorImpl}）は
 * {@link BuriSimpleEngineImpl}と{@link BuriStandardEngineImpl}の 両方のエンジンを保有し、かつ
 * {@link SimpleBuriProcessor}と{@link StandardBuriProcessor} の両方のプロセッサも保有します。
 * このクラスでは実行するフローによって適宜判別して実行します。 以下は、プロセッサとエンジンの関係です。
 * <ul>
 ...
 * </p>
 * <p>
 * フローそれぞれに適用させるプロセッサを判別させる基準は、 そのフローがどちらのプロセッサに登録されているか、を参照して判断します。
 * フローはコンパイルが無事完了した後ぶりはコンパイルを担当したエンジンに
 * コンパイル後のフローを保有しますが、このクラスは、保有する両方のエンジンを参照する事で フローがいずれのプロセッサのものかを判定しています。
 * このフローの登録は、{@code buri/dicon/buri-user.dicon} の {@code BuriEngineConfig}（
 * {@link BuriEngineConfig}）で以下のように設定します。
 * 
 * <pre>
 * &#064;code
 ...
 * </p>
 * <p>
 * なお、このインターフェイスへのDI定義は、 {@code src/main/resources/buri/dicon/buriBase.dicon}
 * に定義されています。 <br/>
 * 拡張や変更が必要な場合は、このdiconファイルを参考にしてください。
 * </p>
 * 
 * @author makotan
 * @author nobeans
...
public interface BuriAutoSelectProcessor {
	 ...
	 * <p>
	 * パスが表すフローのプロセスは、ぶりによってクラスとしてコンパイルされ、 エンジンの属性として保持されます。
	...
	/**
	 * 指定したパスの実行可能なフロープロセスを{@link BuriSimpleEngineImpl} または
	 ...
	 * </p>
	 * <p>
	 * 状態を進めた結果は、トランジションに設定された条件やactionを評価した結果で決定されます。 <br/>
	...
	/**
	 * 状態を次に進めます。 その際に、{@code action}を指定してトランジションの経路を指定する事が可能です。
	 * <p>
	 * 権限管理を行わないフローを実行する場合は、 {@code userData}に{@code null}を指定します。
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * ぶりで管理するデータが持つ状態は通常1つです。 ただし、AND-Splitで分岐するフローの場合、1つのデータが複数の条件に
	 * 所属する事があります。
	 * </p>
	 * 
