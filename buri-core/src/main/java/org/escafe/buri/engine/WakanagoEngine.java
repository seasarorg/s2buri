 ...
 * ぶりの中核を担うエンジンです。
 * <p>
 * ぶりの全ての機能を集約してWrapしているクラスです。
 * フローの読み込みと実行を行う為のメソッドを提供しています。
 * Bao、プロセッサの為の基本的な機能はこのクラスで実装されます。
 * </p>
 * <p>
 * このクラスの高位クラスとして{@link BuriSimpleEngineImpl}と
 * {@link BuriStandardEngineImpl}があり、{@link SimpleBuriProcessor}には
 * {@link BuriSimpleEngineImpl}が、{@link StandardBuriProcessor}には
 * {@link BuriStandardEngineImpl}がそれぞれ適用されます。
 * </p>
 * 
 * @author makotan
 * @author nobeans
...
public interface WakanagoEngine {
	...
	/**
	 * {@link BuriSystemContext}を元に、コンパイル済みのフローの
	 * パッケージを返します。
	 * 
	...
	/**
	 * コンパイル済みのフローのパッケージ（{@link BuriExePackages}）
	 * からコンパイル済みのフローのプロセスを返します。
	 * 
