...
public class BuriPath {
    ...
    /*
     ...
     public BuriPathEntity changePath(String newPath,int pos) {
     BuriPathEntity clonePath = copy();
     clonePath.thisPath = thisPath.changePash(newPath,pos);
     assert realPath.getActivity().size() == 0: "realPathを設定しているのに論理パスだけ設定";
     return clonePath;
     }
     public BuriPathEntity changePath(String newPath,String newRealPath,int pos) {
     BuriPathEntity clonePath = copy();
     clonePath.thisPath = thisPath.changePash(newPath,pos);
     clonePath.realPath = realPath.changePash(newRealPath,pos);
     return clonePath;
     }
     public BuriPathEntity changePath(BuriActivityType newAct,int pos) {
     BuriPathEntity clonePath = copy();
     if(newAct==null) {
     return clonePath;
     }
