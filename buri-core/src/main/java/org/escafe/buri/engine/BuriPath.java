 ...
 * </p>
 * <p>
 * ぶりのパスは以下のようなフォーマットで表現されます。
 * <div><b>{@code パッケージ名.プロセス名[.アクティビティ名]}</b></div>
 * ぶりはXPDLで記述されたフローを実行するルールエンジンの役割を持っていますが、
 * ぶりはフローを都度読み込んで都度解釈するインタープリタ型に処理するのではなく、
 * 一度フローをJavaのバイトコードにコンパイルしてそのバイトコードを実行しています。
 * その場合に生成されるクラスは、以下のように表現されます。
 * <div><b>{@code パッケージID=package.プロセスID=ClassName.アクティビティID=MethodName}</b></div>
 * </p>
 * 
 * @author makotan
 * @author nobeans
...
public class BuriPath {
     ...
     * <p>
     * ぶりではこの値を活用していません。
     * XPDLの読み込み時に取得した値がそのままこのクラスに設定されます。
     * </p>
     */
